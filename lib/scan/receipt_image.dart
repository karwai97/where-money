...
/// A photograph and the size to fit it to. They travel together because
/// [compute] hands its function exactly one argument.
typedef ToResize = ({Uint8List photograph, int longEdge});

/// Shrinks a photograph to its long edge, or hands back exactly what it was
/// given when it is already small enough — re-encoding a picture that does not
/// need it only loses detail off faint thermal print.
Uint8List resizeForStorage(ToResize request) {
  final (:photograph, :longEdge) = request;
...
/// photograph already inside the long edge is stored byte for byte, so what
/// came out of the gallery is not necessarily a JPEG.
