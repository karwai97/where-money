...
void main() {
  ...
  test(
    'adding by hand opens on a blank Extraction with nothing filled in',
    () async {
  ...
  test(
    'a total that does not add up loses its Finding once corrected',
    () async {
      final bloc = against(InMemoryLedgerStore())
        ..add(const ManualExpenseStarted())
  ...
  test(
    'the Check re-runs on a half-typed number without throwing it away',
    () async {
      final bloc = against(InMemoryLedgerStore())
        ..add(const ManualExpenseStarted())
  ...
  test(
    'touching a Line Item records the Line Items as corrected once',
    () async {
      final bloc = against(InMemoryLedgerStore())
        ..add(const ManualExpenseStarted())
        ..add(const LineItemAdded())
        ..add(
          const LineItemCorrected(0, LineItemField.description, 'Nasi lemak'),
        );
  ...
  test(
    'retyping the value a field already holds is not a correction',
    () async {
      final bloc = against(InMemoryLedgerStore())
        ..add(const ManualExpenseStarted())
  ...
  test(
    'committing writes exactly one Expense, recorded as typed by hand',
    () async {
  ...
  test(
    'every field the user touched is named on the committed Expense',
    () async {
      final store = InMemoryLedgerStore();
      final bloc = against(store)..add(const ManualExpenseStarted());
      fillIn(bloc);
  ...
  test(
    'an Expense the user filled in does not arrive needing review',
    () async {
      final store = InMemoryLedgerStore();
      final bloc = against(store)..add(const ManualExpenseStarted());
      fillIn(bloc);
  ...
  group('a manual entry keeps its identity across attempts', () {
    test(
      'a refusal after the write does not duplicate on the next tap',
      () async {
        final store = InMemoryLedgerStore();
        final bloc = against(store)..add(const ManualExpenseStarted());
        fillIn(bloc);
  ...
  test(
    'an empty Line Item row does not settle the Finding it just caused',
    () async {
      final store = InMemoryLedgerStore();
      final bloc = against(store)..add(const ManualExpenseStarted());
      fillIn(bloc);
  ...
  test(
    'retyping a Line Item value it already holds is not a correction',
    () async {
      final bloc = against(InMemoryLedgerStore())
        ..add(const ManualExpenseStarted())
        ..add(const LineItemAdded())
